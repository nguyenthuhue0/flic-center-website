...
import {
  privateEnrollment,
  publicEnrollment,
} from "../../services/Student/RegisterForm";
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (token) {
        ...
        // User đã đăng nhập
        const res = privateEnrollment(dataRegisterCourse, billImage);
        if (res) {
          navigate("/SuccessPage");
        }
      } else {
        const dataPublicRegisterCourse = {
          email,
          fullName,
          phone,
          gender,
          ...
          idNumber,
          idIssuedPlace,
          idIssuedDate: idIssuedDate === "" ? null : idIssuedDate,
          courseId,
          amount,
          paymentMethod,
          note,
        ...
        // User chưa đăng nhập
        const res = await publicEnrollment(dataPublicRegisterCourse, billImage);
    ...
    <div className="bg-gray-50 min-h-screen py-10 px-4">
      <div className="max-w-4xl mx-auto space-y-10">
        ...
        <section className="bg-white rounded-xl shadow-md p-6 md:flex md:justify-between gap-6">
          <div className="md:w-1/2 space-y-2">
            <h3 className="font-bold">
              Bước 1: Điền thông tin cá nhân vào các mục bên dưới:
            </h3>
            <ul className="text-sm text-gray-700 list-disc list-inside">
              ...
              <li>
                Đăng ký nhóm gửi link:{" "}
                <a
                  className="text-blue-500 underline"
                  href="https://by.com.vn/EMxGW"
                >
                  https://by.com.vn/EMxGW
                </a>{" "}
                & gửi file qua mail:{" "}
                <span className="font-semibold">flic@vku.udn.vn</span>
              </li>
              <li>
                Trung tâm sẽ xác nhận đăng ký hợp lệ và hướng dẫn nộp học phí
              </li>
            </ul>
          </div>
          <div className="md:w-1/2 text-center mt-6 md:mt-0">
            ...
            <p className="text-xs mt-1">
              Nội dung CK: <strong>HoVaTen, LyDoNop, SĐT</strong>
              <br />
        ...
        <section className="space-y-8 max-w-md mx-auto bg-white rounded-xl shadow-md p-6 border border-blue-400">
          <h2 className="text-red-500 font-bold text-center text-lg mb-4">
            Form đăng ký
          </h2>
          <form className="space-y-4 max-w-md mx-auto" onSubmit={handleSubmit}>
            ...
            <div>
              <label className="block text-gray-700 font-medium mb-1">
                Email (Yêu cầu nhập đúng địa chỉ email để nhận thông tin khoá
                học) <span className="text-red-500">*</span>
              </label>
              <input
                type="email"
            ...
            <div>
              <label className="block text-gray-700 font-medium mb-1">
                CCCD (Nếu không phải là sinh viên)
              </label>
              <input
                type="text"
                placeholder="CCCD"
            ...
            <div>
              <label className="block text-gray-700 font-medium mb-1">
                Chọn khoá học <span className="text-red-500">*</span>
              </label>
              <select
                value={courseId}
            ...
            <div>
              <label className="block text-gray-700 font-medium mb-1">
                Tên các thành viên (Nếu ĐK theo nhóm)
              </label>
              <input
                type="text"
                value={note}
            ...
            <div>
                ...
                <input
                  ...
                  onChange={(event) => {
                    if (event.target.files.length > 0) {
                      ...
                      setBillImage(file);
                      // ✅ Nếu có file thì set method và thời gian hiện tại
                      setPaymentMethod("BANKING");
                      setPaidAt(new Date().toISOString().replace("Z", ""));
                    }
                  }}
                  className="hidden"
                />
              ...
              {billImage && (
                <div className="mt-2 text-sm text-gray-700">
                  <p>
                    📄 <strong>{billImage.name}</strong> (
                    {(billImage.size / 1024).toFixed(2)} KB)
                  </p>
          ...
          <p className="text-xs text-center mt-4 text-gray-600">
            *Nếu cần được hỗ trợ, hãy liên hệ ngay:{" "}
            <span className="text-red-500 font-semibold">0905-603-504</span> (Cô
            Ngân) <br />
            Hoặc trực tiếp tại:{" "}
            <span className="text-blue-600 font-semibold">Phòng KE105</span> –
